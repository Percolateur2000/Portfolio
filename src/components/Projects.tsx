...
return (
    <div className="bg-slate-300 opacity-90 w-full lg:h-full h-full overflow-auto rounded-xl p-2 pb-36 lg:pb-8 boxAnimation">
        <div className='flex pt-8 items-center flex-col'>
            ...
            {Object(opcproject).map((p: any) => {return <Projectcard key={p.id} gotosite={p.gotosite} imgmodal={p.imgmodal} objective={p.objective} title={p.title} github={t(p.github)} description={p.description} img={p.img} tags={p.tags} url={p.url}/>})}
            </ul>
            ...
            {Object(personalproject).map((p: any) => {return <Projectcard key={p.id} gotosite={p.gotosite} imgmodal={p.imgmodal} objective={p.objective} title={p.title} github={t(p.github)} description={p.description} img={p.img} tags={p.tags} url={p.url}/>})}
            </ul>
        </div>
        </div>
    );
